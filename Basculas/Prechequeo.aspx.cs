...
{
    string cod_rol = "";
    string baseUrl = "http://172.206.251.10:9010/api/";
    ...
    public static string baseUrlStatic = "http://172.206.251.10:9010/api/";
    ...
    protected void lnkBuscar_Click(object sender, EventArgs e)
    {
        ...
        using (WebClient client = new WebClient())
        {
            ...
            try
            {
                ...
                if (data == null || string.IsNullOrEmpty(data.transporter))
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "notFoundAlert", 
                        "Swal.fire('Error', 'La transacción no se encontró en la API.', 'error');", true);
                    return;
                }
                ...
                if (data.statuses == null || (data.statuses.LastOrDefault() == null || data.statuses.LastOrDefault().id != 1))
                {
                    ...
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alreadyCheckedAlert",
                        "Swal.fire('Atención', 'Esta transacción ya ha sido prechequeada.', 'warning');", true);
                    return;
                }

                txt_ingenio.Text        = data.ingenio.user.username;
            ...
            catch (WebException webEx)
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "notFoundAlert", 
                        "Swal.fire('Error', 'El codigo de generación ingresado no existe.', 'error');", true);
                }
            }
            catch (Exception ex)
            {
